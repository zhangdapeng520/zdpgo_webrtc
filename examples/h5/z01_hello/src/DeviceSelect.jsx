...
class DeviceSelect extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            selectedAudioDevice: "", // 当前选择的音频输入设备
            selectedAudioOutputDevice: "", // 当前选择的音频输出设备
            selectedVideoDevice: "", // 当前选择的视频输入设备
    ...
    componentDidMount() {
        // 获取视频对象
        videoElement = this.refPreviewVideo;
    ...
    updateDevices = () => {
        return new Promise((pResolve, pReject) => {
            let videoDevices = []; //视频输入设备列表
            let audioDevices = []; //音频输入设备列表
            let audioOutputDevices = []; //音频输出设备列表
    ...
    render() {
        return (
            <div className="container">
                ...
                {/* 音频输入设备列表 */}
                <Select value={this.state.selectedAudioDevice} style={{width: 150, marginRight: '10px'}}
                ...
                {/* 音频输出设备列表 */}
                <Select value={this.state.selectedAudioOutputDevice} style={{width: 150, marginRight: '10px'}}
                ...
                {/* 视频频输入设备列表 */}
                <Select value={this.state.selectedVideoDevice} style={{width: 150}}
                ...
                {/* 视频预览展示 */}
                <video className="video" ref={(el) => this.refPreviewVideo = el} autoPlay playsInline
