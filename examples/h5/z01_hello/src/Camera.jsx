...
class Camera extends React.Component {
    ...
    handleError(error) {
        if (error.name === 'ConstraintNotSatisfiedError') {
            ...
            message.error(`宽:${v.width.exact} 高:${v.height.exact} 设备不支持`).then(r => {}); // 宽高尺寸错误
        } else if (error.name === 'PermissionDeniedError') {
            message.error('没有摄像头和麦克风使用权限,请点击允许按钮').then(r => {}); // 摄像头权限错误
        }
        message.error(`getUserMedia错误: ${error.name}`, error).then(r => {}); // api错误
    }
